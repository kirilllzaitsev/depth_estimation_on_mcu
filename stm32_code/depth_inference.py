...
def main(x_test, y_test, ser: serial.Serial):
    ...
    for req_img, true_depth in zip(x_test[:test_len], y_test[:test_len]):
        ...
        # map true or pred to same range
        print(f"MAE: {mae}")
...
if __name__ == "__main__":
    ...
    x_test = np.load(
        "/media/master/wext/msc_studies/second_semester/microcontrollers/project/stm32/code/test_data/x_test_depth_32.npy"
    )
    y_test = np.load(
        "/media/master/wext/msc_studies/second_semester/microcontrollers/project/stm32/code/test_data/y_test_depth_32.npy"
    ).squeeze()
